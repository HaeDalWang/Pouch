...
**후속(2026-07-15): 질문 3축 재구성 + 되비춤.** "MBTI처럼 자세히"의 요구를
파고들어, 자세함의 정체가 질문 수가 아니라 **recognition("이게 날 아네")**임을
가렸다(고민.md 논의 착지). 지뢰는 자세함이 아니라 **판정(verdict)** — "너는
X타입 → 이 셋트"로 닫는 순간 2026-07-07에 철회한 "정답 세트"로 미끄러진다.

- **3축** — 역할·방향(자유 입력, 고정 6선택지 폐기: 감지가 자신있게 틀리는
  자리라 역할·궤적을 한 축으로 사람 말로) / 말투(warm·dry·mid → 에이전트가
  따를 지시문 본문) / 경계(자유, 비면 줄 안 만듦 → `boundary`·deny·user 출처).
  옛 "작업 스타일" 자유 한 줄은 말투 선택지로 실체화(Q2 영수증: 건조 고르면
  다음 답이 진짜 건조한지 검증 가능해짐).
- **되비춤(`reflect`)** — 저장 직전 답을 2인칭 서사로 되읽어 확인받는다(판정
  아님). substance는 여기서 나온다. `--style`/`--boundary` 비대화형 플래그 추가.
- **안 건드림** — 스택 스캔 자동화(락된 stack=의도 원칙과 충돌), 셋트 자동선택·
  타입 판정(정답 세트 지뢰 — "3번"으로 미룸). 535 tests.
...
**후속(2026-07-16, ④의 에이전트 쪽 배선 완료): 세션에서 배운 게 기억으로 흘러드는 경로.**
pending 스테이징 계층·정책은 락·구현됐지만(위), 세션에 주입되는 "새 기억 남기는 법"
지침이 그걸 안 가르쳐 유입 경로가 반쪽이었다 — 에이전트는 모든 타입을 그냥 `add`(즉시
INDEXED)하라고만 안내받아, 저마찰 정책과 어긋났다. 지침을 정책에 잇는다:

- **project·reference는 `--pending`으로** — 마찰 없이 스테이징(주입 인덱스를 안 더럽히고
  `pouch evolve` 리뷰 때 일괄 확정). "담을 때"가 아니라 "리뷰 때" 마찰.
- **user·feedback·boundary는 사용자 자리에서만** — 말했거나 확인한 자리에서만 기록,
  자동 침묵 금지. 오독한 일회성이 매 세션 주입되는 standing rule로 굳는 위험(boundary
  deny 오독과 동형)을 지침 차원에서 막는다.
- **새 신호원 아님** — 에이전트가 곧 신호(스스로 배운 걸 남긴다). "반복 질문 자동 감지"는
  별도 신호원이라 여전히 defer(BACKLOG). `render_how_to_remember` 갱신. 615 tests.
...
**후속(2026-07-07, BACKLOG 필수 3 + P1 완료): 커버리지 갭 + boundary 출처.**
"주머니에 담는다"가 선언(6종) 대비 1/3(skill·mcp)만 실제였던 갭을, P1(boundary 출처)과
함께 정면으로 풀었다. 배승도 결정: 범위=읽는 것 3종(command·agent·rule), P1 정면, drop은 방향으로 가름.

- **커버리지** — `import_vendored_doc`로 문서형(skill/agent/command) 일반화 + rule 추가.
  id 해석이 kind로 갈림: skill·agent=frontmatter name, command=파일명(런타임 슬래시 id),
  rule=`<부모>__<stem>`(coding-style.md 5중 충돌을 부모로 구분, store가 평면이라 "/" 대신 "__").
  plugin importer가 commands/·agents/·rules/까지 원자 분해(깨진 조각은 격리 보고). **hook만 남김**
  — 실행 레시피+settings.json 배선이라 성격이 달라(linked에 가까움) 다음 조각.
- **P1 (boundary 출처)** — 도구 파일엔 boundary 선언 필드가 없음을 실물로 확인(산문에서 뽑으면
  deny 오독). 그래서 "딸고 온 boundary"=엔트리에 큐레이터가 붙인 씨앗(`RecommendedBoundary`)이
  설치 시 승격되며 `source:vendored:<id>` 도장. direction(allow/ask/deny) 필드 신설 —
  boundary 설계가 예고한 "deny 오독 시 박는다"를 drop-gate 요구가 당김. drop gate: allow는
  도구와 함께 강등, ask·deny·방향불명은 잔존+경고, 사람이 건 것(user)은 무관하게 잔존.
  용어는 [GLOSSARY.md](docs/GLOSSARY.md)(경계의 방향/출처)에 등재. 322 tests.

**후속(2026-07-07, 필수 3의 남은 조각 완료): 훅 담기 — 커버리지 6종 전부 열림.**
훅 = "이 사건이 나면 이 명령을 실행하라"는 실행 배선. 읽는 지식이 아니라서 도구연결(mcp)과
같은 결(linked + 조리법)로 담고, 설치는 파일 복사가 아니라 settings.json 배선이다.

- **담기** — hooks.json의 그룹 하나 = 항목 하나. 그룹 id를 정체로(파일명 불가 글자는 -로),
  id 없으면 사건+대상으로 결정적 합성(훅은 사용 기록과 이름 맞출 필요가 없어 합성이 안전 —
  스킬의 name 요구와 다른 이유). plugin 분해가 hooks/hooks.json을 자동으로 집어감.
  플러그인 경유 훅은 surface=plugin(이미 실행 중이라 또 배선하면 이중 실행 — mcp와 동형).
- **설치 관문(배승도 결정)** — 훅은 내 컴퓨터에서 명령을 실제 실행하므로 설치 때 **명령
  원문을 반드시 표시 + 동의**. --yes로 물음은 건너뛰되 원문 출력은 항상 남김.
- **신호 없는 종류 보호** — 훅·규칙·에이전트는 usage.jsonl에 안 찍힐 뿐 일하고 있어
  "안 쓰임"을 판별할 수 없다 → drop 후보에서 제외(boundary를 위생에서 뺀 것과 같은 이유:
  신호 없음 ≠ 안 쓰임). 이로 인해 생기는 "내릴 문 없음" 구멍은 `pouch catalog uninstall`
  신설로 메움(수동 내리기 — 모든 종류에 유효, 카탈로그·개인화는 보존).
- 실물 ECC hooks.json(26개) 왕복 검증. 340 tests.

**후속(2026-07-07, BACKLOG 필수 4 완료): 시작 세트 — 콜드 스타트 온보딩.**
완전 처음인 사용자는 카탈로그가 비어 init이 빈손이었다(맞춰줄 물건 자체가 없음).
구상 락(같은 날, 배승도 3결정): 세트=출처까지 담음 / 내장 1개+`~/.pouch/sets/` /
init이 세트 먼저 제안.

- **세트 = 선곡표** — JSON 하나: 이름·제목·매칭 토큰 + 항목들(출처 경로 + 올릴 id 목록).
  가져오기·설치는 전부 기존 카탈로그 경로 재사용(새 보관통·새 설치 메커니즘 없음).
- **내장 세트 1호** — "해달왕의 AWS DevOps 엔지니어 세트": 실물 AWS 플러그인에서
  13스킬+1연결을 담고, DevOps에 맞는 7개만 표면에(sdk 변종은 창고에만 — 진화가 판단).
- **입구 둘** — init(역할·스택 토큰 매칭, 가장 잘 맞는 세트 통째 제안, 거절 시 기존 흐름) +
  `pouch set list/apply`. 같은 이름은 사용자 세트가 내장을 이김(개인 우선). 훅이 올라가면
  명령 원문 항상 출력. 깨진 세트 파일·증발한 출처·없는 id는 격리 보고(인질 금지).
- **동승 수리** — init의 연결 단계가 기억 주입만 걸고 사용 기록 훅을 안 걸던 반쪽 배선을
  발견·수리("init 하나로 온보딩 끝"이 되는 순간 진화 데이터가 안 쌓이는 진짜 구멍이 됨).
- 빈 환경에서 init 한 번 → 세트 제안 → 14개 담고 7개 올림 → 훅 2개 연결, 실물 검증. 355 tests.

**정정(같은 날): 내장 세트 내용물 철회 — 지어낸 세트는 내장에 못 들어온다.**
오너 검증에서 내장 1호(임의 큐레이션 7개)가 실사용과 다름이 드러났다. "DevOps면 이거
쓰겠지"로 고른 세트는 pouch가 반대하며 출발한 "남이 만든 정답 세트"와 같다. 배관(세트
형식·apply·init 제안·CLI)은 유지하고 내용물만 비움. 원칙을 `sets/builtin/README.md`에
박음: **내장 세트 = 실사용에서 굳힌 것만.** 굳히는 문(세트 내보내기 = state+usage에서
세트 파생)은 백로그로 — 수 주 실사용 후 배승도의 진짜 주머니를 export한 것이 내장 1호가 된다.

**진화 (A)→(B) 관리레이어 전환 (2026-07-13 락, [[pouch-evolution-management-layer]]):**
진화 = 행위(drop/reattach) 아니라 **조언**. 표면은 누가 소유하든 pouch는 그 위에서
관측하고 사람 말 조언만 얹는다(강제 안 함). 근거: ownership 3값·surface:plugin이 이미
(B)를 말하고 있었고 진화 엔진만 마지막 (A) 잔존물. ✅ 조각 1(모델 A→B, 조언 분류·배선)
+ "이거 써봐" 2a(매칭 태그→설명 토큰) + 스코핑 A(plugin 번들 doc 4종→관측 스텁) 완료.

**후속(2026-07-13 락): import 관문 (다) — 가리키기만, 실사용이 진입시킨다.**
통과기준 ②("한 번 import에 카탈로그가 안 넘친다")의 미완을 닫는 raft 직전 필수 문
([[pouch-import-gate-policy]]). import는 번들을 **소스로만** 등록(카탈로그 진입 0개),
사용자가 실제로 쓰면 다음 관측 패스에서 올바른 kind·ownership으로 카탈로그 자동 진입.
import는 뭘 담을지 판단 안 함(전량 자동 등록만 끊음 = 런치템플릿 함정 회피). 2단
store(`sources/`=스테이징 / `catalog/`=진입, CatalogStore 재사용), 진입 문턱 1회
(번들 일부러 들인 사실이 우연 방어), 스테이징 범위=전부. 콜드 스타트는 세트가 시드.
**194 마이그레이션은 관문 완료 후 별도 슬라이스**(진입 기준 서면 그 기준으로 일괄).
✅ 조각 1(promote 코어) + 2(sources_dir·reconcile) + 3(import→소스 배선) 완료.
✅ 조각 4(demote·migrate 코어, promote/reconcile의 거울상: 안 쓴 카탈로그→소스 강등,
alias canonicalize·has_usage_signal 방어) + 5(`pouch catalog migrate` CLI — 사용자가
...
- **다음(오너 확정 2026-07-17): owned 임베드** — 직접 만든 도구의 본문을 세트 파일에
  품어, owned도 세트에 담기게. 배승도의 실수요(회사에서 커스텀 스킬·훅 노하우를
  세트로 동료에게 나누기)가 근거. raft 평가의 실사용 시나리오이기도.
...
## Phase 4.7 — 홈그라운드 굳히기 (정렬 · 도구통)

> 목표: Phase 4.6이 "쌓인 걸로 제안한다"까지 왔다면, 여기선 **주입하는 쪽**과
> **채우는 쪽**의 남은 구멍을 메운다. 스코프 락([[pouch-scope-boundary]], 2026-07-14)이
> 자리를 갈랐다 — 환경·기억(작업 전/후)은 pouch의 홈그라운드, 작업 *중* 실행 품질은
> 다른 하네스 몫. 정렬은 "작업 전에 목표를 박는" 쪽이라 홈그라운드에 남는다.

- [x] **① 정렬 체크포인트 (2026-07-14~15)** — 대화가 접히면 목표가 흐려지고, 에이전트가
  목표를 매번 다시 요약하면 어긋남이 누적된다. 목표 한 줄을 사이드카에 박고
  (`pouch checkpoint set/show/clear`), 세션 시작 주입에 **되짚는 규약**을 함께 싣는다:
  관찰 가능한 트리거 3종(갈림길·재계획·계획 밖 시작)과 4슬롯 `⟦ALIGN⟧` 블록.
  - **판단 위임 금지** — "중요한가?"를 느낌으로 묻지 않는다. 사건이 일어났는지만 본다.
  - **◆GOAL은 복사, 요약 아님** — 재요약하는 순간 흐려짐이 재발한다(기능의 존재 이유).
  - **손잡이 매번** — 요약 후 멈추지 않되(흐름 유지) 개입 여지만 남긴다.
  - **언어 갈림** — 라벨·마커는 영어/ASCII(모델 준수 안정·정규식 안전), 값은 한글(읽는 사람).
  - **⚠️ 주입 내림 (2026-07-21)** — 며칠 실사용 결과 도움이 안 됐다. 발동이 모델의 자율
    준수에만 걸려(강제 훅 부재) 따로 놀았고, 매 세션 규약 40여 줄 맥락 비용만 냈다. 세션
    시작 주입에서 규약을 뺐다. **떨어진다 ≠ 삭제된다** — 명령·render.py는 휴면으로 남겨,
    훅으로 발동을 강제할 길이 생기면 되올린다. 702 tests. (계획했던 Phase-2 강제 훅은 미착수)
- [x] **② 목표 자리를 폴더별로 (2026-07-21)** — 앵커가 `~/.pouch/anchor.json` 하나뿐이라
  A 프로젝트 목표가 B 프로젝트 세션 시작에 그대로 주입되던 사고. memory·catalog가
  이미 쓰는 global/project 2계층을 앵커도 따르게 했다. **폴백 없음** — 프로젝트 안이면
  글로벌을 대신 읽지 않는다(허용하는 순간 오염 경로가 되살아난다). 테스트가 격리
  구멍도 같이 드러냄(테스트가 실제 repo `.pouch/`에 쓰고 있었다). 704 tests.
- [x] **③ 도구통 훑기 (2026-07-21)** — 채우는 쪽 구멍: 사용자가 `catalog import <경로>`로
  경로를 손수 찍어야 해서, 그런 게 있는 줄 모르는 사람은 영원히 빈손이었다.
  `pouch catalog sweep`이 **에이전트별 도구 위치를 알고 훑어 소스(대기실)까지만** 재운다.
  - **하네스 어댑터** — Claude(`~/.claude/`)·Codex(`~/.codex/`)·Kiro(`~/.kiro/`)를
    어댑터로 분리(`hosts/`). 훑기 코드엔 에이전트 이름이 안 박혀 있다 — 새 하네스는
    어댑터만 더하면 자동으로 낀다. **BACKLOG D2 갱신의 근거**(아래 참조).
  - **소스까지만** — 장부 진입·표면 설치는 안 한다. 진입 기준은 그대로 실사용
    ([[pouch-import-gate-policy]]의 관문을 안 뚫는다). 그래서 훑기는 안전하고 자동일 수 있다.
  - **자동은 아니되 안 보이지도 않게** — 첫 화면이 딱 한 번, 아직 안 훑었으면 알린다.
- [x] **④ 종류별 제 서랍 (2026-07-21)** — 표면에 올릴 때 에이전트가 스킬 자리에 놓이던
  것. Claude가 이미 나눠둔 자리(skills/·agents/·commands/)를 그대로 재사용한다.

**산출물:** 목표가 안 새고, 이미 깔린 도구가 저절로 대기실까지 온다.
...
> **설계 락(2026-07-16 제안 → 2026-07-17 오너 확인): [docs/RAFT-DESIGN.md](docs/RAFT-DESIGN.md)**
> — 공유 매체=Git 기반 공유 레지스트리(**v0 범위: git 아는 엔지니어 팀**, 비개발자
> 확장 시 어댑터 추가), 공유 단위=세트만(기억·경계·사용기록은 로컬 전용). v0 슬라이스: `set pull`.

- [x] pouch 구성 export / import — **세트 단위 v0 완료.** 내보내기(`set export`, 표면을
  세트로 굳히기)와 가져오기(`set import`, 남의 세트를 들이기)가 짝. 아래 후속 참조.
- [~] 팀 공유 레지스트리 — **받는 쪽(`set pull`) 완료.** 주는 쪽은 v0 git-native(export
  후 repo에 커밋/PR) — publish 헬퍼는 실사용 마찰 확인 후. 아래 후속 참조.
- [x] 개인 메모리와 공용 구성의 경계 정의 — RAFT-DESIGN "개인/공용 경계" 표로 확정
  (나가는 것=세트뿐, 기억·경계·사용기록·프로젝트 `.pouch/`는 절대 안 나감). 오너 확인 2026-07-17.
- [x] **owned 임베드 — ✅ 완료 (2026-07-18).** 직접 만든 도구의 본문을
  세트 파일에 품어 owned도 세트에 담기게. 배승도의 실수요(회사 노하우 세트 공유)가 근거.
  **방식 락(배승도, 2026-07-18): 인라인** — 오너의 말: *"(가)는 이러한 개인들의
  스킬/도구들은 에초에 Set을 만들때 같이 한번에 넣어버리면된다는거잔아?? 다른 도구들
  처럼 별도 패키징이 아니라?? … ㅇㅇ 가로 가자"* — 세트 JSON 한 파일이 자기완결
  (본문을 안에 통째로). 별도 꾸러미 동봉(두 개 주기)은 탈락. v0는 파일 하나짜리
  도구부터, 여러 파일짜리는 건너뛰고 이유 보고(인질 금지).
  **동승 수리(같은 날)** — 세트 CLI가 embed 항목을 몰라 `set export` 보고에서
  IndexError로 죽던 것, 올릴 개수를 잘못 세던 것, 임베드 항목에 가짜 출처를 적던 것.
...
**후속(2026-07-16, 세트 가져오기 v0): raft의 첫 문 — 남이 굳힌 것을 받는 쪽.**
내보내기(export)가 "내 표면을 세트로 굳혀 내보낸다"면, 가져오기(import)는 그 거울상 —
남의 세트 파일을 검증해 내 `~/.pouch/sets/`로 들인다. 매체는 로컬 파일(v0): URL·레지스트리는
다음. 세트는 매니페스트(선곡표)라 코드가 아니라서 들이기만으론 아무것도 실행·설치되지
않는다 — 실제 적용은 `pouch set apply`가 하고 동의는 거기서 받는다(신뢰 경계는 apply의 관문).
이 컴퓨터에 없는 출처는 정직히 보고하고 apply 때 건너뛴다(인질 금지). `pouch set import <경로>`.
export→import→apply 전 루프가 돈다. 616 tests.
