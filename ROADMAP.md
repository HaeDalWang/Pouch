...
- [ ] **② 색인** — 등록된 저장소를 훑어 도구 목록을 소스 대기실의 별도 티어로.
  sweep의 원격판: **아는 배치만**(skills/·agents/·commands/·.mcp.json …) 훑고
  모르는 구조는 건너뛰고 보고(인질 금지). 정체는 `<저장소>/<도구>`(이름 충돌 방어 —
  규칙 `__`·레지스트리 `<author>/<name>`과 같은 패턴). **색인은 가리키기만** —
  카탈로그 진입은 기존 관문(실사용/명시 설치)을 그대로 지난다.
