...
## Phase 4.8 — 다듬기 넓히기: 저장소 모델 (제안 · 오너 확인 대기)
...
**조각 (제안 · 오너 확인 대기 — 순서 = 의존 순서):**

- [ ] **① 저장소 등록** — `pouch repo add/list/remove <git주소>`. `set pull`의
  clone 패턴 재사용(git shell-out·멱등 pull·기존 인증). **여러 개 등록 가능** —
  지금 registry가 하나뿐이고 바꾸기를 거부하는 제약의 일반화.
