...
- [ ] ① `pouch catalog` CLI — import(plugin/skill/.mcp.json 자동 판별)·list·install·sync 배선.
  install은 drop 후 **재부착의 공식 입구**이기도 하다(Phase 4 가역성의 마지막 배선)
- [ ] ② `pouch` 상태 화면 승격 — 민낯 `pouch` 한 화면: 주머니에 뭐가 있고 / 최근 뭘 썼고 /
  뭐가 오르내릴 후보인지. 백로그 doctor의 축소 승격(가장 싸고 즉시 체감)
- [ ] ③ attach 제안 — evolve에 "당겨올 것" 섹션. 카탈로그에 있는데 표면에 없고+쓴 것 → 재부착 제안 /
  카탈로그 밖인데 자주 쓰는 것 → 편입 안내(제안만, 자동 없음 — drop과 같은 원칙)
