...
## 두 번째 조각 — 프로젝트 사용 → 주머니 편입 안내 (완료)

레인 2a의 프로젝트 로컬 사용 신호를 프로젝트 카탈로그와 이었다. `pouch report`가
프로젝트 사용을 둘로 가른다(전역 리포트의 most_used/outside와 같은 구조):

- **📁 이 프로젝트에서 많이 쓴 것** — 이미 (전역·프로젝트) 카탈로그에 있는 것.
- **📁 이 프로젝트에서 쓰는데 주머니 밖** — 어느 카탈로그에도 없는 것 →
  `pouch catalog import --project`로 담으라 안내. 읽기전용(행동은 사용자 명령).

## 밖에 두는 것 (다음)

- **표면/install 스코핑** — MCP는 프로젝트 `.mcp.json`이 자연스럽지만, **스킬은 Claude가
  전역(`~/.claude/skills`)이라 프로젝트 표면 자리가 없다.** 이 비대칭은 별도 설계.
