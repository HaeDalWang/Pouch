...
**쉬운 말** — 규칙의 세기나 우선순위 등급이 **아니다**. "자기 이름의 정식 입구를
가진 개념"이라는 개발자 은어다. 남의 집에 세들어 살던 개념에 자기 명패를 달아주는
것 — "경계 1급화"는 경계가 `pouch memory add -t boundary`라는 곁다리 길이 아니라
`pouch boundary add/list/remove`라는 **자기 명령**을 갖게 됐다는 뜻일 뿐이다.

**왜 이 개념이 생겼나** — 프로그래밍 용어 "first-class citizen(1급 시민)"에서 왔다.
pouch에서는 "중요한 개념인데 다른 개념의 부속으로 사는" 상태(경계가 기억의 한
종류로만 존재)를 벗어나 자기 동사·자기 화면을 갖는 일을 가리키는 데 썼다.

**헷갈리기 쉬운 점** — "1급 가드레일 = 무조건 지켜야 하는 규칙"으로 읽기 쉽다
(오너가 실제로 이렇게 읽고 걸렸다, 2026-07-16). 아니다 — 규칙이 얼마나 세게
읽히는지는 1급화와 무관하게 방향(allow/ask/deny)이 정하고, 그건 1급화 전부터
그대로다. pouch가 아무것도 막지 않는다는 것도 그대로다. 1급화로 변한 것은
오직 **거는 입구와 보는 화면**뿐이다.

**위치** — 결정 문서 [BOUNDARY-PROMOTION.md](BOUNDARY-PROMOTION.md) ·
명령 구현 [src/pouch/boundary/](../src/pouch/boundary/) + cli.py 등록
