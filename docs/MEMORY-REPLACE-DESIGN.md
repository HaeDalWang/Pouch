...
> #2·#4는 오너 확인을 거쳐 닫혔다(2026-07-16). #1·#3·#5는 아직 열려 있는
> 진짜 미결이다 — 급하지 않아 다음 조각에서 닫는다.

1. **provenance 보존** — `originSessionId`를 새 필드로 남길까, 버릴까. **열림.**
2. **세션로그 자동 판별** — ✅ **닫힘. 오너 확인: 배승도, 2026-07-16.** 오너의 되풀이:
   *"기억을 옮길 때 효율적으로 하기 위해, (일지임이 확실한 것은) 서랍으로 가서
   나중에 찾는 것."* 규칙: 이름에 유효한 YYMMDD가 박힌 project는 ARCHIVED, 없으면
   PENDING(`_looks_dated`). 오분류돼도 recall로 복구 가능(실측 리뷰 큐 45→30).
3. **repo 역매핑 실패 처리** — project 기억의 원래 repo가 사라졌으면? (global 강등 +
   메모 vs skip vs 별도 보관) **열림.**
4. **write 지침 문구·위치** — ✅ **닫힘. 오너 확인: 배승도, 2026-07-16.** 오너의 되풀이:
   *"pouch로 기억을 하게 되면서 정상적으로 동작하기 위해 기억을 되짚는(적는) 방법을
   적어놓은 것."* 형태: 고정 구역(경계·체크포인트 아래)에 영어 지침
   (`render_how_to_remember`), 채우는 값은 사용자 언어. **단 오너가 후속 크럭스를
   짚음: "'기억할 게 생기면'의 판단 조건이 엄청 중요하다"** — 이 조건은 현재
   에이전트 재량(무른 장치)이고, 과소 적재(안 적음)의 방어가 없다는 문제로 #5와
   직결된다. #5를 열 때 이 짚음에서 출발할 것.
5. **write 경로 장기안 착수 시점** — 단기 지침으로 버티다 언제 PostToolUse 훅 설계로
   갈지. **열림 — #4에서 오너가 짚은 "적는 조건" 크럭스가 이 설계의 입구다.**
