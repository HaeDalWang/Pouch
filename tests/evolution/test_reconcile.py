...
# --- demote_candidates: promote_candidates의 거울상(카탈로그→소스 강등 선택) ---


def test_unused_catalog_tool_is_demote_candidate() -> None:
    # 카탈로그에 있는데 안 쓴 도구 → 소스로 강등 대상(옛 import가 직행시킨 잉여).
    stats: dict[str, UsageStat] = {}
    result = demote_candidates(stats, catalog_ids={"unused"})
    assert result == ["unused"]


def test_used_catalog_tool_stays() -> None:
    # 실제로 쓴 건 강등 안 함 — 카탈로그에 정당히 속한다(진입 유지: exa 등).
    stats = {"exa": _stat(count=1)}
    result = demote_candidates(stats, catalog_ids={"exa"})
    assert result == []


def test_not_in_catalog_is_not_demote_candidate() -> None:
    # 카탈로그에 없는 건 강등할 게 없다(소스에만 있는 스테이징은 대상 아님).
    stats: dict[str, UsageStat] = {}
    result = demote_candidates(stats, catalog_ids=set())
    assert result == []


def test_demote_candidates_sorted() -> None:
    # 결정적 순서(정렬) — 194 마이그레이션 보고·테스트 안정.
    stats: dict[str, UsageStat] = {}
    result = demote_candidates(stats, catalog_ids={"zeta", "alpha", "mid"})
    assert result == ["alpha", "mid", "zeta"]


def test_demote_mirrors_promote() -> None:
    # 대칭 확인: 쓴 것은 진입 유지, 안 쓴 것은 강등 — 둘이 겹치지 않는다.
    stats = {"exa": _stat()}
    catalog_ids = {"exa", "unused"}
    kept = demote_candidates(stats, catalog_ids=catalog_ids)
    assert kept == ["unused"]  # exa는 쓴 것이라 강등 제외
